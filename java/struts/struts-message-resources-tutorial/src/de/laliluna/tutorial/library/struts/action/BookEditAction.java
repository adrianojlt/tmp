...
public class BookEditAction extends DispatchAction {
	...
	public ActionForward saveBook(
		...
		if(bookEditForm.getAuthor().equals(""))
			actionErrors.add("author", new ActionError("error.field", messageResources.getMessage("label.author", request)));
		if(bookEditForm.getTitle().equals(""))
			actionErrors.add("author", new ActionError("error.field", messageResources.getMessage("label.title", request)));
